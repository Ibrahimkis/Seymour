...
const ChapterInspector = ({ chapterId }) => {
  ...
  const stats = useMemo(() => {
    ...
    words.forEach(w => {
      ...
      const s = clean.replace(/(?:[^laeiouy]es|ed|[^laeiouy]e)$/, '').match(/[aeiouy]{1,2}/g);
    ...
    // 5. Word Frequency
    const stopWords = new Set(['the','and','to','of','a','in','that','is','was','he','she','it','for','on','with','as','his','her','at','be','this','have','from','or','one','had','by','word','but','not','what','all','were','we','when','your','can','said','there','use','an','each','which','she','do','how','their','if']);
    const frequency = {};
    words.forEach(w => {
      const clean = w.toLowerCase().replace(/[^a-z]/g, '');
      if (!stopWords.has(clean) && clean.length > 2) frequency[clean] = (frequency[clean] || 0) + 1;
    });
    const topWords = Object.entries(frequency).sort((a, b) => b[1] - a[1]).slice(0, 5);
        ...
        {activeTab === 'analytics' && stats && (
          <>
            <div style={gridStyle}>
              ...
              <div style={metricBox}>
                <div style={metricVal}>{stats.readTime} <span style={{fontSize:'10px'}}>min</span></div>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                <div style={{ fontSize: '32px', fontWeight: 'bold', color: getGradeColor(stats.gradeLevel) }}>
                  {stats.gradeLevel}
                </div>
            ...
            <div style={{ marginBottom: '20px' }}>
              ...
              <div style={{ fontSize: '13px', color: 'var(--text-main)', marginBottom: '5px' }}>
                Count: <span style={{ fontWeight: 'bold', color: stats.adverbs.length > (stats.wordCount / 100) * 3 ? '#e74c3c' : '#2ecc71' }}>{stats.adverbs.length}</span>
              </div>
              <div style={{ fontSize: '11px', color: '#666', fontStyle: 'italic' }}>(Aim for &lt; 3%)</div>
            </div>
              ...
              {stats.topWords.map(([word, count], i) => (
                <div key={i} style={rowStyle}>
                  ...
                  <div style={barContainer}>
                    <div style={{ width: `${Math.min(100, (count / stats.topWords[0][1]) * 100)}%`, height: '100%', background: 'var(--accent)', opacity: 0.5, borderRadius: '2px' }}></div>
                  </div>
                  <span style={{ fontSize: '11px', color: '#666', minWidth: '20px', textAlign: 'right' }}>{count}</span>
                </div>
              ))}
            </div>
          </>
...
const tabContainerStyle = { display: 'flex', borderBottom: '1px solid var(--border)', flexShrink: 0 };
...
const metricBox = { flex: 1, background: 'var(--bg-app)', border: '1px solid var(--border)', borderRadius: '4px', padding: '10px', textAlign: 'center' };
...
const labelStyle = { fontSize: '10px', fontWeight: 'bold', color: 'var(--accent)', marginBottom: '8px', textTransform: 'uppercase' };
const divider = { height: '1px', background: 'var(--border)', margin: '20px 0' };
const rowStyle = { display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '6px', fontSize: '13px' };
const barContainer = { flex: 1, height: '6px', background: 'var(--bg-app)', borderRadius: '2px', overflow: 'hidden' };
